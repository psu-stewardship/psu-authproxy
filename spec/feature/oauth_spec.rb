...
feature 'OAuth Authorization Flow' do
    ...
    scenario 'oauth token flow' do
        ...
        visit oauth_authorization_url(client_id: uri_params['client_id'][0], redirect_uri: uri_params['redirect_uri'][0], response_type: uri_params['response_type'][0])
        code = page.response_headers['Location'].split('=')[1]
        expect(page.status_code).to eq(302)

    end

    scenario 'bad client id' do
        Rails.application.env_config['REQUEST_URI'] = '/oauth/authorize'
        Rails.application.env_config['REMOTE_USER'] = 'djb44'
        page.driver.instance_variable_get(:@options)[:follow_redirects] = false
        visit oauth_authorization_url(client_id: 'bogus', redirect_uri: uri_params['redirect_uri'][0], response_type: uri_params['response_type'][0])
        expect(page.body).to include('OAuth authorization required')
    end

    scenario 'no REMOTE_USER' do 
    ...
    # scenario '(null) remote user' do
    #     Rails.application.env_config['REMOTE_USER'] = "(null)"
    #     visit oauth_authorization_url(client_id: uri_params['client_id'][0], redirect_uri: uri_params['redirect_uri'][0], response_type: uri_params['response_type'][0])
    #     expect(page.status_code).to eq(302)
    #     expect(page.response_headers['Location']).to include('webaccess.psu.edu')
